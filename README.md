...
# Основні можливості
## Можливості прошивки
* Керування через WIFI з телефона під управленням Android/IOs (з допомогою бібліотеки RemotXY http://remotexy.com). Проект інтерфейсу додатку можна знайти тут http://remotexy.com/ru/editor/68c04610a9f8e383eab6f33242d87893/
* Керування одним двигуном постійного струму
* Керування двома сервомоторами (Руль, то додаткове обладнання)
...
* Автоматичне уввімкнення/вимкнення поворотів під час їзди (задається поріг спрацьовування від 0% до 100%вивороту коліс)
...
* Окрема кнопка для стоянкових вогнів
* Окрема кнопка для ближнього світла фар
* Окрема кнопка для дальнього світла фар
* Окрема кнопка для протитуманних вогнів
* Окрема кнопка для світлового маячка
* Окремі кнопка для ручного включення поворотів
* Окремі кнопка для звукового сигналу (частота задається в насройках)
* Повзунок для керування сервоприводом
...
1) Сервопривід - Керуючий вивід => D5 (GPIO14)
2) Сервопривід навісного - Керуючий вивід => D8 (GPIO15)
...
4) Освітлення
	...
	* P3 - Лівий поворот
	* P4 - Правий поворот
	* P5 - лампа заднього ходу
	* P6 - лампа стопсигналу
	* P7 - Протитуманки
9) Бузер RX

## Схема
### Обвязка модуля ESP8266
![ESP8266 - Wiring diagram](/img/Processor.bmp)

### Підключення освітлення
![Light - Wiring diagram](/img/Light.bmp)

### Підключення моторів
![Motors - Wiring diagram](/img/Motors.bmp)

### Співвідношення пінів Wemos D1 mini та модуля ESP8266
![Wemos - Wiring diagram](/img/Socket.bmp)
